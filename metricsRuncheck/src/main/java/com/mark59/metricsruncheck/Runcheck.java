...
public class Runcheck  implements CommandLineRunner 
{
	...
	public static void parseArguments(String[] args) {
		...
		options.addOption("d", "databasetype",			true, "Load data to a 'h2' or 'mysql' database (defaults to mysql");			
		...
		options.addOption("h", "dbserver",				true, "Server hosting the MySQL database where results will be held (defaults to localhost)" );
		options.addOption("p", "dbPort",    			true, "Port number for the database where results will be held (defaults to 3306 for MySQL, 5432 for Postgress" );		
		options.addOption("s", "dbSchema",				true, "database schema (MySQL terminology) / database name (Posgress terminology) defaults to metricsdb" );
		options.addOption("u", "dbUsername",   			true, "Username for the MySQL database (defaults to admin)" );				
		options.addOption("w", "dbpassWord",   			true, "Password for the MySQL database" );				
		options.addOption("y", "dbpassencrYpted",		true, "Encrypted Password for the MySQL database (value as per the encryption used by mark59-server-metrics-web 'Edit Server Profile' page)");				
		options.addOption("q", "dbxtraurlparms",		true, "Any special parameters to append to the end of the databuse URL (include the ?). Eg \"?allowPublicKeyRetrieval=truee&useSSL=false\" (the quotes are needed to escape the ampersand)");				
		...
		if ( !Mark59Constants.H2.equalsIgnoreCase(argDatabasetype) && !Mark59Constants.MYSQL.equalsIgnoreCase(argDatabasetype) && !Mark59Constants.PG.equalsIgnoreCase(argDatabasetype)  ) {
			formatter.printHelp( "Runcheck", options );
			printSampleUsage();
			throw new RuntimeException("The database type (d) argument must be set to 'h2', 'pg' or 'mysql' ! (or not used, in which case 'h2' is assumed)");  
		}
		...
		// argDatabasetype used to set h2 or MySQL db usage via application.properties: spring.profiles.active={spring.profile}
	...
	public static void printSampleUsage() {
		...
		System.out.println( "   java -jar runcheck.jar -a MY_COMPANY_BIG_APP -i C:/jmeter-results/BIGAPP -r \"run ref 645\" -p 3309  ");
		...
		System.out.println( "   The metricsdb database is hosted locally on a MySql instance assigned to port 3309 (default user/password of admin/admin) : "  );
		System.out.println( "   java -jar runcheck.jar -a MY_COMPANY_BIG_APP -i C:/templr/BIGAPP/AnalysisSession/AnalysisSession.mdb -r \"run ref 644\" -p 3309 -t LOADRUNNER" );
		System.out.println();
	}
	...
	public void run(String... args) throws Exception {
		...
		if (Mark59Constants.H2.equalsIgnoreCase(currentDatabaseProfile)) {
			...
			System.out.println("*   Please use a MySQL database for more formal work.                                 *" );			
			System.out.println("*                                                                                     *" );			
			System.out.println("***************************************************************************************" );
			System.out.println();			
		}		
